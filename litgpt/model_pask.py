...
from litgpt.qkv_reassemble import qkv_reassemble
...
class GPT(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.audio_adapter = nn.Linear(1280, config.n_embd)
    ...
    def fill_in_audio_feature(self, input_embeddings, batch_size, audio_feat_encoder, output_lens, audio_pos):
        if batch_size != len(output_lens):
            raise ValueError(f"batch_size = {batch_size} != {len(output_lens)} = len(output_lens)")
        
        count = 0
        for batch_idx in range(batch_size):
            if output_lens[batch_idx] != -1:
                input_embeddings[batch_idx, audio_pos[batch_idx]: audio_pos[batch_idx] + output_lens[batch_idx], :] = audio_feat_encoder[count: count+output_lens[batch_idx], :]
                count += output_lens[batch_idx]

        return input_embeddings

    def get_audio_encoder(self, audio_encoder):
        self.audio_encoder = audio_encoder

    def concat_audio_feature(self, lens_audio_feats, input_lens):
        return_lens_audio_feats = []
        return_input_lens = []
        count = 0 
        for i in range(len(lens_audio_feats)):
            if lens_audio_feats[i] != -1:
                return_lens_audio_feats.append(lens_audio_feats[i])
                return_input_lens.append(input_lens[i])
                count += 1
        return return_lens_audio_feats, return_input_lens, count
    ...
    def forward(
        ...
        # print(f"input)pos_maxp1: {input_pos_maxp1}")
        T = idx.size(1)
        print(T)
        ...
        #     print(key)
        #     print(type(value))
        if get_audiofeat_in_forward:
            raise NotImplementedError("get_audiofeat_in_forward is not implemented yet")
        else:
            if T > audio_tokens_per_chunck:

                if x.size(0) != 1:
                    raise ValueError("inference mode, it is not supported for batch size > 1")
                #print(x)
                print(f"replace [{T-(audio_tokens_per_chunck+1)}:{T-1}] with audio feature")
                x[0, T - (audio_tokens_per_chunck + 1): T-1, : ] = self.audio_adapter(audio_feat)
                #print(x)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if use_flash:
            ...
            q = q.transpose(1, 2).continuous()  # (B, T, nh_q, hs)
            k = k.transpose(1, 2).continuous()  # (B, T, nh_k, hs)
            v = v.transpose(1, 2).continuous()  # (B, T, nh_v, hs)
